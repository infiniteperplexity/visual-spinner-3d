...
//constructor?
Vector.prototype.define = function(def) {
	if (def===undefined) {
		return undefined;
	}
	if(def.x && def.y && def.z) {
...
JSONizer.prototype.parseProp = function(json) {
	...
	def = Constants.deconstantize(def);
	return def;
}
//should we add a "sparsify" parameter?
JSONizer.prototype.stringifyProp = function(p) {
	var def = {}
	var e;
	for (var i = HOME; i<=PROP; i++) {
		e = {};
		if (i==PROP) {
			if (p.prop.radius !== 1) {
				e.radius = p.prop.radius;
			}
		} else {
			// what about negative radius?
			if (p[ELEMENTS[i]].radius > TINY) {
				e.radius = p[ELEMENTS[i]].radius;
			}
		}
		if (p[ELEMENTS[i]].zenith != QUARTER) {
			e.radius = p[ELEMENTS[i]].zenith;
		}
		if (p[ELEMENTS[i]].azimuth > TINY) {
			e.radius = p[ELEMENTS[i]].azimuth;
		}
		if (e.length > 0) {
			def[ELEMENTS[i]] = Constants.constantize(e);
		}
	}
	if (p.twist !== 0) {
		def.twist = p.twist;
	...
	if (p.axis !== undefined && !p.axis.nearly(WALL)) {
		def.axis = p.axis
	}
	if (p.name) {
		def.name = p.name;
	...
	def = Constants.constantize(def);
	def.moves = [];
	for (var i = 0; i<p.move.submoves.length; i++) {
		def.moves[i] = JSONizer.prototype.stringifyMove(p.move.submoves[i].definition);
	}
...
Prop.prototype.defineMoves = function(def) {
	...
	for (var i = 0; i<def.moves.length; i++) {
		move = MoveFactory.prototype.build(def[i].recipe, def[i]);
		this.addMove(move);
	}
}
Prop.prototype.definePosition = function(def) {
	...
	this.twist = def.twist;
	this.grip = def.grip;
	this.choke = def.choke;
	this.bend = def.bend;
	//need to implement
	this.axis = Vector.define(def.axis)
	this.propname = def.propname;
...
JSONizer.prototype.parseMove = function(json) {
	var def = JSON.parse(json);
	return MoveFactory.prototype.build.fromRecipe(def);
}

JSONizer.prototype.stringifyMove = function(move) {
	var def = move.definition;
	//need to handle constants and defaults in the correct order
	def = Constants.constantize(def);
	var defaults = MoveFactory.prototype.recipes[movename].defaults;
	for (key in defaults) {
		if (def[key]===defaults[key]) {
			def[key] = undefined;
		}
	}
	return JSON.stringify(def,null,2);
...
MoveFactory.prototype.build = function(movename, options) {
	var augmented = this.augment(options);
...
function(options) {
	...
	if ((options.hybrid == true || options.extend < SMALL) && options.entry == TWELVE) {
		move.align("hand",SIX);
	} else {
		move.align("hand", options.entry);
	}
